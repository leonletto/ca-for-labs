...
if [[ "$sanExists" == "0" ]]; then
    ...
    openssl ca -batch -days "$CERT_DAYS" -passin file:"$ca_passfile" -extensions no_san_server_cert -config openssl.cnf  -in "$csrFileName" \
    ...
    #Sign the Cert
    echo "Signing the certificate with the CA"
    openssl ca -batch -days "$CERT_DAYS" -passin file:"$ca_passfile" -extensions SAN -extfile ./options.cnf -config openssl.cnf  -in "$csrFileName" \
     -out certs/"$domain".crt
fi
